...
from modules.PBR import PBRSet
...
class ProcessingThread(QThread):
    ...
    def run(self):
        ...
        pbr_set = PBRSet(self.albedo, self.metallic, self.roughness)
        if self.type == "correcting":
            ...
            self.roughness_corrected = pbr_set.roughness_corrected
...
class GUI(QMainWindow):
    ...
    def handle_processing_signal(self, message):
        ...
        if message == "Done":
            ...
            if self.processing_thread.type == "correcting":
                ...
                self.pbr_set.roughness_corrected = self.processing_thread.roughness_corrected
    ...
    def setup_widgets(self):
        ...
        self.roughness_group = self.findChild(QGroupBox, 'roughnessGroup')
        self.roughness_icon = self.findChild(QPushButton, 'roughnessIcon')
        self.setroughness_button = self.findChild(QToolButton, 'setroughnessButton')
        self.roughness_path_input = self.findChild(QLineEdit, 'roughnessPathInput')
        ...
        self.setroughness_button.clicked.connect(lambda: self.load_texture(self.roughness_icon, self.roughness_path_input, image_type="roughness"))
    ...
    def load_texture(self, icon, texture_input, is_set_image=False, image_type="albedo"):
        ...
        try:
            ...
            if image_type == "ao":
                self.status_label.setText("Ambient Occlusion was loaded successfully")
            ...
            if image_type == "metallic":
                self.status_label.setText("Metallic texture was loaded successfully")
                self.metallic_image = Image.open(filepath[0])

            if image_type == "roughness":
                self.status_label.setText("Roughness texture was loaded successfully")
                self.roughness_image = Image.open(filepath[0]).convert("RGB")
    ...
    def correct_albedo(self):
        ...
        if (self.mode == "combined" or self.mode == "metallic") and self.is_compensating and self.roughness_image is None:
            self.roughness_path_input.setStyleSheet("border-color: #a03c3c;")
            valid_inputs = False
        ...
        if valid_inputs:
            ...
            self.processing_thread.roughness = self.roughness_image
    ...
    def save_textures(self):
        if self.pbr_set is not None:
            ...
            albedo_filename = os.path.basename(self.albedo_path_input.text()).split(".")[0]
            roughness_filename = os.path.basename(self.roughness_path_input.text()).split(".")[0]
            try:
            ...
            if selected_directory is not None:
                self.pbr_set.save(selected_directory, albedo_filename, roughness_filename)
                self.status_label.setText("Successfully saved")
            else:
                self.pbr_set.save(os.path.dirname(self.albedo_path_input.text()), albedo_filename, roughness_filename)
                self.status_label.setText("Successfully saved")
